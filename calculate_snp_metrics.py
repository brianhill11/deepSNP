...
def main():
    ...
    for location in real_snps:
        ...
        if num_snps % 100000 == 0 and num_snps > 0:
            cur_time = time.time()
            log.info("Num real SNPs processed: %s (%s percent)", num_snps, float(num_snps)/len(real_snps))
            log.info("Elapsed time: %s", cur_time - start_time)
            log.info("Avg num reads per window: %s", float(total_num_reads)/float(num_snps))
            log.info("Avg num usable reads per window: %s", float(total_num_usable_reads)/float(num_snps))
            log.info("Avg num overlapping reads: %s", float(total_num_overlapping_reads)/float(num_snps))
            log.info("Avg num usable overlapping reads: %s", float(total_num_usable_overlapping_reads)/float(num_snps))
    ...
    for location in candidate_snps:
        ...
        if num_snps % 100000 == 0 and num_snps > 0:
            cur_time = time.time()
            log.info("Num candidate SNPs processed: %s (%s percent)", num_snps, float(num_snps)/len(candidate_snps))
            log.info("Elapsed time: %s", cur_time - start_time)
            log.info("Avg num reads per window: %s", float(total_num_reads)/float(num_snps))
            log.info("Avg num usable reads per window: %s", float(total_num_usable_reads)/float(num_snps))
            log.info("Avg num overlapping reads: %s", float(total_num_overlapping_reads)/float(num_snps))
